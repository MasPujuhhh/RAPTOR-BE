...
class Controller{
    static async add(req, res){
        try {
            ...
            const hasil = await ReportLabel.create({id:nanoid(), nama, color})
            res
    ...
    static async list(req, res){
        try {
            ...
            const hasil_count = await sequelize.query(`SELECT * from master_report_label where "deletedAt" is null ${filter};`, { type: QueryTypes.SELECT });
            const hasil = await sequelize.query(`SELECT * from master_report_label where "deletedAt" is null ${filter} order by nama limit ${per_page} offset ${offset};`, { type: QueryTypes.SELECT });
    ...
    static async detail(req, res){
        try {
            const hasil = await ReportLabel.findOne({ where: { id: req.params.id } });
            if (!hasil) {
                const err = new Error('Label tidak ditemukan!!')
    ...
    static async editUser(req, res){
        try {
            ...
            const Category = await ReportLabel.findByPk(id)
            if (!Category) {
                const err = new Error('Label tidak ditemukan!!')
                err.code = HttpStatusCode.NotFound
                throw err
